        ...
        <div style={{ flex: 0.5, padding: '12px 20px', borderRight: '1px solid var(--border-subtle)', display: 'flex', flexDirection: 'column' }}>
          ...
          {word.recordingUrl ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginTop: '2px' }}>
              <button onClick={e => { e.stopPropagation(); handlePlayRecording() }} style={{ width: '32px', height: '32px', borderRadius: '50%', flexShrink: 0, backgroundColor: playingRecording ? 'rgba(251,191,36,0.1)' : 'rgba(45,212,191,0.08)', border: playingRecording ? '1px solid rgba(251,191,36,0.3)' : '1px solid rgba(45,212,191,0.25)', color: playingRecording ? '#fbbf24' : '#2dd4bf', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          {word.suggestions.map((s, idx) => {
            ...
            return (
              <div key={idx} style={{ flex: '1 1 0', minWidth: '200px', display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 10px', borderRadius: '6px', border: '1px solid var(--border-subtle)', backgroundColor: 'var(--surface-2)' }}>
                <button onClick={e => { e.stopPropagation(); handlePlaySuggestion(idx) }} style={{ width: '24px', height: '24px', borderRadius: '50%', flexShrink: 0, backgroundColor: isPlaying ? 'rgba(251,191,36,0.1)' : 'rgba(45,212,191,0.08)', border: isPlaying ? '1px solid rgba(251,191,36,0.3)' : '1px solid rgba(45,212,191,0.25)', color: isPlaying ? '#fbbf24' : '#2dd4bf', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <>
            {pronunciation && (
              <div style={{ flexGrow: 1, flexShrink: 1, minWidth: '120px', position: 'relative' }}>
                ...
                <button onMouseDown={e => { e.preventDefault(); e.stopPropagation(); handlePlay() }} style={{ position: 'absolute', left: '6px', top: '50%', transform: 'translateY(-50%)', width: '24px', height: '24px', borderRadius: '50%', backgroundColor: playingPreview ? 'rgba(251,191,36,0.1)' : 'rgba(45,212,191,0.08)', border: playingPreview ? '1px solid rgba(251,191,36,0.3)' : '1px solid rgba(45,212,191,0.25)', color: playingPreview ? '#fbbf24' : '#2dd4bf', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                {generateContextSentences(word.word).map((sentence, sentIdx) => {
                  ...
                  return (
                    <div key={sentIdx} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '5px 8px', borderRadius: '4px', backgroundColor: 'var(--surface-3)', border: '1px solid var(--border-subtle)' }}>
                      <button onMouseDown={e => { e.preventDefault(); e.stopPropagation(); handlePlaySentence(sentIdx as 0 | 1 | 2) }} style={{ width: '20px', height: '20px', borderRadius: '50%', flexShrink: 0, backgroundColor: isPlaying ? 'rgba(251,191,36,0.1)' : 'rgba(45,212,191,0.08)', border: isPlaying ? '1px solid rgba(251,191,36,0.3)' : '1px solid rgba(45,212,191,0.25)', color: isPlaying ? '#fbbf24' : '#2dd4bf', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
